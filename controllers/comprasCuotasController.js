    ...
    listar: async (req, res) => {
      try {
        ...
        for (const compra of compras) {
          ...
          compra.proxima_cuota_iso = proxFecha.toISOString().slice(0, 10);
          compra.vencida = compra.estado !== 'completada' && compra.proxima_cuota_iso < hoyStr;
        }
      ...
      try {
        ...
        await db.run(
          `INSERT INTO movimientos_caja (tipo, concepto, categoria, monto, metodo_pago, fecha, turno)
           VALUES (?, ?, ?, ?, ?, ?, ?)`,
          ['egreso', `Cuota ${compra.cuotas_pagadas + 1}/${compra.cant_cuotas} - ${compra.descripcion}`, compra.categoria, compra.monto_cuota, compra.medio_pago || 'manual', timestamp, turnoByHora(timestamp)]
        );
