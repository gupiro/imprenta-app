...
module.exports = (db) => {
  return {
    ...
    crear: async (req, res) => {
      try {
        const { tipo_comprobante, numero_comprobante, fecha_emision, cuit_proveedor, razon_social, descripcion, categoria_contable, forma_pago, estado, fecha_vencimiento_pago, notas, proveedor_id } = req.body;

        // Obtener items del formulario
        const itemsDescripcion = Array.isArray(req.body.item_descripcion) ? req.body.item_descripcion : [req.body.item_descripcion || ''];
        const itemsCantidad = Array.isArray(req.body.item_cantidad) ? req.body.item_cantidad : [req.body.item_cantidad || 1];
        const itemsPrecioUnitario = Array.isArray(req.body.item_precio_unitario) ? req.body.item_precio_unitario : [req.body.item_precio_unitario || 0];
        const itemsAlicuota = Array.isArray(req.body.item_alicuota) ? req.body.item_alicuota : [req.body.item_alicuota || 21];
        ...
        for (let i = 0; i < itemsDescripcion.length; i++) {
          if (!itemsDescripcion[i] || itemsDescripcion[i].trim() === '') continue;

          const cantidad = parseFloat(itemsCantidad[i]) || 1;
          const precioUnitario = parseFloat(itemsPrecioUnitario[i]) || 0;
          const alicuota = parseFloat(itemsAlicuota[i]) || 21;

          const montoNeto = cantidad * precioUnitario;
          const montoIva = (montoNeto * alicuota) / 100;
          const subtotal = montoNeto + montoIva;

          items.push({
            descripcion: itemsDescripcion[i],
            cantidad,
        ...
        if (items.length === 0) {
          req.flash('error', 'Debe agregar al menos un item');
          return res.redirect('/finanzas/facturas-recibidas');
        }
        ...
        // Calcular alícuota promedio para almacenar
        const alicuotaPromedio = items.length > 0 ? (totalIva / totalNeto * 100) : 21;

        const result = await db.run(
          `INSERT INTO facturas_recibidas (tipo_comprobante, numero_comprobante, fecha_emision, cuit_proveedor, razon_social, descripcion, categoria_contable, monto_neto, alicuota_iva, monto_iva, monto_total, forma_pago, estado, fecha_vencimiento_pago, notas, periodo, proveedor_id)
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [tipo_comprobante, numero_comprobante, fecha_emision || obtenerFechaLocal().fecha, cuit_proveedor || '', razon_social, descripcion || '', categoria_contable || '', totalNeto, alicuotaPromedio, totalIva, montoTotal, forma_pago || '', estado || 'pendiente', fecha_vencimiento_pago || '', notas || '', periodo, proveedor_id || null]
        );
        ...
        for (const item of items) {
          await db.run(
            `INSERT INTO facturas_recibidas_items (factura_id, descripcion, cantidad, precio_unitario, monto_neto, alicuota_iva, monto_iva, subtotal)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
            [facturaId, item.descripcion, item.cantidad, item.precio_unitario, item.monto_neto, item.alicuota_iva, item.monto_iva, item.subtotal]
          );
        }

        req.flash('success', `✅ Factura ${numero_comprobante} registrada (${items.length} items - Monto: $${montoTotal.toLocaleString('es-AR', {minimumFractionDigits: 2})})`);
    ...
    editar: async (req, res) => {
      try {
        const { id } = req.params;
        const { tipo_comprobante, numero_comprobante, fecha_emision, cuit_proveedor, razon_social, descripcion, categoria_contable, monto_neto, alicuota_iva, forma_pago, estado, fecha_vencimiento_pago, notas } = req.body;
        ...
        const alicuotaNum = parseFloat(alicuota_iva) || 21;
        const montoIva = (montoNetoNum * alicuotaNum) / 100;
        const montoTotal = montoNetoNum + montoIva;

        await db.run(
          `UPDATE facturas_recibidas
           SET tipo_comprobante = ?, numero_comprobante = ?, fecha_emision = ?, cuit_proveedor = ?, razon_social = ?, descripcion = ?, categoria_contable = ?, monto_neto = ?, alicuota_iva = ?, monto_iva = ?, monto_total = ?, forma_pago = ?, estado = ?, fecha_vencimiento_pago = ?, notas = ?
           WHERE id = ?`,
          [tipo_comprobante, numero_comprobante, fecha_emision, cuit_proveedor || '', razon_social, descripcion || '', categoria_contable || '', montoNetoNum, alicuotaNum, montoIva, montoTotal, forma_pago || '', estado || 'pendiente', fecha_vencimiento_pago || '', notas || '', id]
        );

        req.flash('success', '✅ Factura actualizada');
        res.redirect('/finanzas/facturas-recibidas');
      } catch (err) {
