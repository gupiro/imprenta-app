    ...
    listar: async (req, res) => {
      try {
        ...
        for (const gf of gastosFijos) {
          ...
          // Sobreescribir estado/ícono según progreso de pago (tiene prioridad sobre fecha)
          if (gf.porcentaje_pagado >= 100) {
            gf.estado = 'verde';
            gf.iconoEstado = '🟢';
          } else if (gf.porcentaje_pagado > 0) {
            gf.estado = 'amarillo';
            gf.iconoEstado = '🟡';
          } else {
            gf.estado = 'rojo';
      ...
      try {
        ...
        const cajaResult = await db.run(
          `INSERT INTO movimientos_caja (tipo, concepto, categoria, monto, metodo_pago, fecha, turno)
           VALUES (?, ?, ?, ?, ?, ?, ?)`,
          ['egreso', `Gasto Fijo: ${gf.nombre}`, gf.categoria, gf.monto, 'manual', timestamp, turnoByHora(timestamp)]
        );
    ...
    registrarPagoParcial: async (req, res) => {
      try {
        ...
        const cajaResult = await db.run(
          `INSERT INTO movimientos_caja (tipo, concepto, categoria, monto, metodo_pago, fecha, turno)
           VALUES (?, ?, ?, ?, ?, ?, ?)`,
          ['egreso', `Pago Parcial - ${gf.nombre}`, gf.categoria, parseFloat(monto), metodo_pago || 'manual', timestamp, turnoByHora(timestamp)]
        );
