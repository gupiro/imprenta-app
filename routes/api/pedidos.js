...
module.exports = (db) => {
  ...
  // Get full pedido details (for modal in caja diaria)
  router.get('/:id', async (req, res) => {
    try {
      const pedidoId = parseInt(req.params.id);
      const pedido = await db.get(
        'SELECT p.*, c.name as cliente_nombre FROM pedidos p LEFT JOIN clients c ON p.client_id = c.id WHERE p.id = ?',
        pedidoId
      );
      ...
      const productos = await db.all(
        'SELECT * FROM productos WHERE pedido_id = ? ORDER BY id',
        pedidoId
      ) || [];
